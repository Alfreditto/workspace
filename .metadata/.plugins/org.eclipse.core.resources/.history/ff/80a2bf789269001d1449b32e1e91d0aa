...
public class Ejercicio7 {
	public static void main(String[] args) {
		...
		try {
			...
			int idCliente = 100;
			ResultSet resul = sentencia.executeQuery("select nombre from clientes where id = 103;");
			while(resul.next()) {
			...
			resul = sentencia.executeQuery("select count(v.idventa), sum(v.cantidad * p.pvp) from clientes c join ventas v on c.id = v.idcliente join productos p on p.id = v.idproducto where c.id = "+idCliente+" group by c.id;");
			while (resul.next()) {
